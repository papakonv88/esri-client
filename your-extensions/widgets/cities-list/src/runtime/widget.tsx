...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  useEffect(() => {
    if (selectedCity?.filename) {
      dispatch(
        appActions.widgetStatePropChange(widgetId, "city", selectedCity),
      );
    }
      ...
      {isOpen ? (
        <div className={"timeline-wrapper"}>
          ...
          <div className="d-flex justify-content-center">
            <Label className="mt-2">Αναζήτηση Πόλης</Label>
          </div>
          <TextInput
          ...
          <Scrollable className={"mt-1 list-wrapper"}>
            <div className={"cities-list"}>
              {filteredCities.length > 0 ? (
                ...
                <p style={{ fontWeight: "bold" }}>Δεν βρέθηκαν αποτελέσματα</p>
              )}
            </div>
          </Scrollable>
        </div>
        ...
        <div className="exbmap-ui">
          <CustomIconButton handleModal={handleIsOpen} />
        </div>
      )}
    </>
  );
