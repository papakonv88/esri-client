...
const DiagramContainer = ({diagramID, type, data, filenameToDownload}) => {
    const chartRef = useRef<am4charts.XYChart | null>(null)

    let symbol = '', diagramTitle = '', preFilenameToDownload = ''

    switch (type) {
        case 'temp':
            symbol = '°C'
            diagramTitle = 'Θερμοκρασία'
            preFilenameToDownload = 'Tmean'
            break
        case 'rain':
            symbol = 'mm'
            diagramTitle = 'Βροχόπτωση'
    ...
    useLayoutEffect(() => {
        ...
        const s2 = createSeries(chart, 'RCP2.6 (αυστηρό)', 'dateRange', 'rcp26')
        const s3 = createSeries(chart, 'RCP4.5 (ενδιάμεσο)', 'dateRange', 'rcp45')
        const s4 = createSeries(chart, 'RCP8.5 (ακραίο)', 'dateRange', 'rcp85')
